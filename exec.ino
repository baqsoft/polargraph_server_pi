...
boolean exec_executeBasicCommand(String &com)
{
  ...
  if (com.startsWith(CMD_CHANGELENGTH))
    ...
    eeprom_resetEeprom();
  else if (com.startsWith(CMD_PWMOFF)) 
    exec_pwmOff();
  else if (com.startsWith(CMD_PWMON))
    exec_pwmOn();
  else if (com.startsWith(CMD_SETTHRUST)) 
    exec_setThrust();
  else if (com.startsWith(CMD_HOMING))
    exec_homing();
  else if (com.startsWith(CMD_CHANGELENGTAHEAD))
    exec_changeLengthAhead();
  else
    executed = false;
...
void exec_pwmOff() {
  analogWrite(PEN_HEIGHT_SERVO_PIN, 0);
}

void exec_pwmOn() {
  analogWrite(PEN_HEIGHT_SERVO_PIN, currentThrust);
}

void exec_setThrust() {

  long thrust = atol(inParam1);
  currentThrust = thrust * 255 / 100;
  Serial.print(F("New thrust (0,255): "));
  Serial.println(currentThrust);
}

void exec_homing() {

  long runSpeed = currentMaxSpeed / 2;
  usingAcceleration = false;
  reportingPosition = false;
  while(limitSwitch1 || limitSwitch2) {
    long relLengthA = 0;
    if (limitSwitch1) {
      relLengthA = -1;
    }
    ...
    changeLengthRelative(relLengthA, relLengthB, true);
  }
  usingAcceleration = true;
  reportingPosition = true;
  Serial.println(F("On the home position!"));
}

void exec_changeLengthAhead() {
  float endA = multiplier((float)atof(inParam1));
  float endB = multiplier((float)atof(inParam2));
  //int maxSegmentLength = atoi(inParam3);
  int pointsPassed = atoi(inParam3);
  int pointsAhead = atoi(inParam4);
  ...
  {
    exec_drawBetweenPoints(startA, startB, endA, endB, 2, pointsPassed, pointsAhead);
  }
}
...
void exec_changeLengthDirect()
{
  ...
  {
    exec_drawBetweenPoints(startA, startB, endA, endB, maxSegmentLength, 0, 0);
  }
}  
...
long lastDirectSpeed = 0;
void exec_drawBetweenPoints(float p1a, float p1b, float p2a, float p2b, int maxSegmentLength, int pointsPassed, int pointsAhead)
{
...
//      Serial.println(linesegs);
      ...
      // do the move
      lastDirectSpeed = desiredSpeed(linesegs + pointsAhead, lastDirectSpeed, currentAcceleration*4);
      
//      Serial.print("Setting speed:");
//      Serial.println(runSpeed);
      
      motorA.setSpeed(lastDirectSpeed);
      motorB.setSpeed(lastDirectSpeed);
      changeLength(pA, pB);
